...
- **AGREE** - 基于假设/保证契约的组合验证（外部插件，由 Collins Aerospace 开发，非 OSATE2 主仓库组件）
...
OSATE2 主仓库提供代码生成**可行性检查器**：

- **`org.osate.codegen.checker`** - 验证 AADL 模型是否满足代码生成的前提条件（端口连接类型、线程调度属性等）
- 实际代码生成（ARINC653配置、POK配置等）由外部工具（如 Ocarina）完成，不在 OSATE2 主仓库中
