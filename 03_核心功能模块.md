...
### 6.1 代码生成检查器（Code Generation Checker）

OSATE2 主仓库中的代码生成支持仅限于**前置条件检查**，不包含实际的代码生成器。

**`org.osate.codegen.checker`** 的功能：
- 检查线程的调度协议（`Scheduling_Protocol` 属性）是否已指定
- 检查连接的端口类型是否符合代码生成要求
- 验证组件绑定的完整性（处理器绑定、内存绑定）
- 检查 ARINC653 相关属性的一致性

**实际代码生成**（ARINC653配置、POK/RTOS配置）由独立的外部工具完成（如 Ocarina、AADLib），这些工具使用 OSATE2 解析出的模型作为输入，但其源码不在 OSATE2 主仓库中。

**相关模块：**
- `org.osate.codegen.checker` - 代码生成前置条件检查器
